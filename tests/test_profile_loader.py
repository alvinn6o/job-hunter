...
class TestWebToPythonCompatibility:
    ...
    def test_min_score_field_not_used_by_scraper(self, full_profile):
        """minScore is used by email_digest, not scrape.py. Ensure it doesn't break loading."""
        path = write_profile(full_profile)
        result = load_profile(path)
        # Should load without error even though minScore isn't consumed by load_profile
